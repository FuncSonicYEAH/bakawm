...
pub fn render_and_download(
    ...
    elements: impl Iterator<
        Item = impl smithay::backend::renderer::element::RenderElement<
    ...
    let mut frame = renderer
        .render(&mut target, size, output_transform)
    ...
    for element in elements {
        ...
        let src = element.src();
        element
            .draw(&mut frame, src, geo, &[geo], &[], None)
...
pub fn clear_dmabuf(
    ...
    let size = size
        .to_logical(1, smithay::utils::Transform::Normal)
        .to_physical(smithay::utils::Scale::from(1));
...
pub fn render_to_dmabuf(
    ...
    elements: &[impl smithay::backend::renderer::element::RenderElement<
        smithay::backend::renderer::gles::GlesRenderer,
    >],
    ...
    let mut target = renderer
        ...
        .map_err(|e| anyhow::anyhow!("error binding dmabuf: {:?}", e))?;

    let clear_color = smithay::backend::renderer::Color32F::TRANSPARENT;
    let res: RenderOutputResult<'_> = damage_tracker
        .render_output_with_states(renderer, &mut target, 0, elements, clear_color, states)
        .map_err(|e| anyhow::anyhow!("error rendering to dmabuf: {:?}", e))?;
