...
#[cfg(feature = "xdp-gnome-screencast")]
pub fn draw_pointer<R, F>(
    renderer: &mut R,
    pointer_element: &PointerElement,
    pointer_location: smithay::utils::Point<f64, smithay::utils::Logical>,
    output_geometry: smithay::utils::Rectangle<i32, smithay::utils::Logical>,
    output: &Output,
    push: &mut F,
) where
    R: Renderer + ImportAll + ImportMem,
    R::TextureId: Texture + Clone + Send + 'static,
    F: FnMut(PointerRenderElement<R>),
{
    let output_scale = Scale::from(output.current_scale().fractional_scale());
    let pointer_pos = pointer_location - output_geometry.loc.to_f64();
    let pointer_pos = pointer_pos.to_physical_precise_round(output_scale);

    let elements: Vec<PointerRenderElement<R>> =
        pointer_element.render_elements(renderer, pointer_pos, output_scale, 1.0);
