...
impl ScreenCast {
    async fn create_session(
        ...
        let session = Session::new(session_id, self.ipc_outputs.clone(), self.to_state.clone());
...
impl Session {
    ...
    pub async fn stop(
        ...
        if let Err(err) = self.to_state.send(ScreenCastToState::StopCast {
            session_id: self.id,
        }) {
            warn!("error sending StopCast to state: {err:?}");
        }
    ...
    async fn record_monitor(
        ...
        let target = StreamTarget::Output {
...
impl Drop for Session {
    fn drop(&mut self) {
        let _ = self.to_state.send(ScreenCastToState::StopCast {
            session_id: self.id,
        });
    }
}
...
impl Stream {
    ...
    fn start(&self, ctxt: SignalEmitter<'static>) {
        ...
        let msg = ScreenCastToState::StartCast {
        ...
        if let Err(err) = self.to_state.send(msg) {
            warn!("error sending StartCast to state: {err:?}");
        }
    }
}
