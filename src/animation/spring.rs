...
//!
//! Ported from niri (GPL-3.0-or-later), which is based on libadwaita (LGPL-2.1-or-later)
//! and RBBAnimation (MIT).
...
mod tests {
    ...
    #[test]
    fn overdamped_spring_equal_from_to() {
        let spring = Spring {
            from: 0.,
            to: 0.,
            initial_velocity: 0.,
            params: SpringParams::new(1.15, 850., 0.0004),
        };
    ...
    #[test]
    fn overdamped_spring_duration() {
        let spring = Spring {
            from: 0.,
            to: 1.,
            initial_velocity: 0.,
            params: SpringParams::new(6., 1200., 0.0004),
        };
        let _ = spring.duration();
        let _ = spring.clamped_duration();
        let _ = spring.value_at(Duration::ZERO);
